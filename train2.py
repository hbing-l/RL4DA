...
project_path = '/home/hanbing/RL4DA'
data_path = '/home/hanbing/RotNIST/data/angle'
model_path = '/home/hanbing/RL4DA/ckpt3'
...
def compute_distribution_discrepancy(representations1, representations2, device, epsilon=1e-8):
    representations1 = replace_nan_with_zero(representations1)
    representations2 = replace_nan_with_zero(representations2)
    m = ot.dist(representations1, representations2, metric='euclidean')
    m = replace_nan_with_zero(m)
    m_max = m.max()
    if m_max == 0:
        m_max = epsilon
    m = m / m_max
    ...
    w_dist = ot.sinkhorn2(a, b, m, 0.5)
    if torch.isnan(w_dist).any():
        logging.error("Wasserstein 距离计算中检测到 NaN 值")
        raise ValueError("Wasserstein 距离计算中有 NaN 值")
    return w_dist

# Helper function to compute reward based on discrepancy measurement
def compute_reward(source_feature_, selected_domain, target_feature_, device, gamma=0.6):
    ...
    if len(selected_domain) == 0:
        reward = gamma * compute_distribution_discrepancy(source_feature, target_feature, device)
        ...
        reward = - gamma * compute_distribution_discrepancy(inter_feature, source_feature, device) - (1 - gamma) * compute_distribution_discrepancy(inter_feature, target_feature, device)
        ...
        reward = - gamma * compute_distribution_discrepancy(current_inter_feature, prev_inter_feature, device) - (1 - gamma) * compute_distribution_discrepancy(current_inter_feature, target_feature, device)
        logging.info(f"selected domain list>1, reward:{reward}")
    return reward
...
def eval(sdg, predictor, dataloader, intermediate_loaders, device):
    ...
    with torch.no_grad():
        ...
        for i, loader_idx in enumerate(intermediate_idx):
            loader = intermediate_loaders[loader_idx]
...
def train(sdg, predictor, device, source_loader, intermediate_loaders, target_loader, test_intermediate_loaders, test_target_loader, epochs, n_j):
    optimizer_sdg = torch.optim.Adam(sdg.parameters(), lr=1e-4)
    optimizer_predictor = torch.optim.Adam(predictor.parameters(), lr=1e-4)
    ...
    for epoch in range(epochs):
        ...
        for idx in range(n_j):
            ...
            for i, loader_idx in enumerate(intermediate_idx):
                ...
                distance2 = compute_distribution_discrepancy(inter_feature2_, target_feature2_, device)
        ...
        if epoch % 10 == 0:
            torch.save({"state_dict":predictor.state_dict(), "epoch":epoch}, os.path.join(model_path, f'predictor_epoch{epoch}.bin'))
            torch.save({"state_dict":sdg.state_dict(), "epoch":epoch}, os.path.join(model_path, f'sdg_epoch{epoch}.bin'))

        if acc > best_acc:
            best_acc = acc 
            torch.save({"state_dict":predictor.state_dict(), "epoch":epoch}, os.path.join(model_path, 'predictor_best_model.bin'))
            torch.save({"state_dict":sdg.state_dict(), "epoch":epoch}, os.path.join(model_path, 'sdg_best_model.bin'))
...
file_handler = logging.FileHandler(os.path.join(project_path, 'log/{}.log'.format(time1)), mode='a')
...
writer = SummaryWriter(os.path.join(project_path, 'runs/{}/'.format(time1)))
...
sdg = SDG(input_dim=64, hidden_dim=32, output_dim=1).to(device)
predictor = Predictor().to(device)

RotNISTLoader = RotNISTDataLoader(data_path, 0, [18, 36, 54, 72], 90, True, 'train', 256)
source_loader, intermediate_loaders, target_loader = RotNISTLoader.get_loaders()

RotNISTLoader = RotNISTDataLoader(data_path, 0, [18, 36, 54, 72], 90, True, 'test', 256)
test_source_loader, test_intermediate_loaders, test_target_loader = RotNISTLoader.get_loaders()

RotNISTLoader = RotNISTDataLoader(data_path, 0, [18, 36, 54, 72], 90, True, 'valid', 256)
...
# Eval the models
valid_sdg = SDG(input_dim=64, hidden_dim=32, output_dim=1).to(device)
valid_predictor = Predictor().to(device)
logging.info("Loading checkpoint ... ...")
saved_dict = torch.load(os.path.join(model_path, "predictor_best_model.bin"))
...
saved_dict = torch.load(os.path.join(model_path, "sdg_best_model.bin"))
...
logging.info(f"Accuracy on valid target domain: {acc}")
