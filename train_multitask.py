...
def train(sdg, predictor, device, source_loader, intermediate_loaders, target_loader, test_intermediate_loaders, test_target_loader, epochs, n_j):
    ...
    for epoch in range(epochs):
        ...
        if epoch % 10 == 0:
            torch.save({"state_dict":predictor.state_dict(), "epoch":epoch}, os.path.join(project_path, f'ckpt2/predictor_epoch{epoch}.bin'))
            torch.save({"state_dict":sdg.state_dict(), "epoch":epoch}, os.path.join(project_path, f'ckpt2/sdg_epoch{epoch}.bin'))

        if acc > best_acc:
            best_acc = acc 
            torch.save({"state_dict":predictor.state_dict(), "epoch":epoch}, os.path.join(project_path, 'ckpt2/predictor_best_model.bin'))
            torch.save({"state_dict":sdg.state_dict(), "epoch":epoch}, os.path.join(project_path, 'ckpt2/sdg_best_model.bin'))
...
saved_dict = torch.load(os.path.join(project_path, "ckpt2/predictor_best_model.bin"))
...
saved_dict = torch.load(os.path.join(project_path, "ckpt2/sdg_best_model.bin"))
